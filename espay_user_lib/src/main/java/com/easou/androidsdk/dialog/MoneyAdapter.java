...
import com.easou.androidsdk.login.service.BaseResponse;
...
class MoneyAdapter extends BaseAdapter {
    ...
    public MoneyAdapter(Context context, List<MoneyInfo> list) {
        mContext = context;
            ...
            public void onClick(View v) {
                if (listener != null && list.size() > 0 && list.get(position).getStatus() == 1) {
                    ...
                    ThreadPoolManager.getInstance().addTask(new Runnable() {
                        @Override
                        public void run() {
                            final GainMoneyInfo moneyTask = EucService.getInstance(mContext).
                                    getMoneyTask(playerId, list.get(position).getId(), CommonUtils.getServerId(mContext));
                            ((Activity) mContext).runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    ESToast.getInstance().ToastShow(mContext, moneyTask.getDesc());
                                }
                            });
                        }
                    });
